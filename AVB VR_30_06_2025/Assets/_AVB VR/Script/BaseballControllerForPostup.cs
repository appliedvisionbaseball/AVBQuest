...
public class BaseballControllerForPostup : MonoBehaviour
{
    ...
    [Header("Postup Drill")]
    public bool isVirtualOrPitchtypeDrill = false;
    public PostupDrillResultHandler postupDrillResultHandler;
    public PitchTypeDrillResultHandler pitchTypeDrillResultHandler;
    ...
    void Update()
    {
        if (isTweening)
        {
            ...
            if ((InputBridge.Instance.RightTriggerDown || Input.GetKeyDown(KeyCode.R) || (InputBridge.Instance.LeftTriggerDown) || Input.GetKeyDown(KeyCode.L)) && !isVirtualOrPitchtypeDrill)
            {
            ...
            if (tweenTimer == 0 && !isEndLoop)
            {
                ...
                //if (pitchType == "Slider")
                //{
                //    tempBallPos = new Vector3(baseballTransform.position.x + 3f, baseballTransform.position.y - 0.04f, baseballTransform.position.z + 2f);
                //}
                //else
                {
    ...
    public void ValueSetupOnUI()
    {
        ...
        if (postupDrillResultHandler != null)
        {
            speedOMeterObject.SetActive(false);
        }
        else if (!isVirtualOrPitchtypeDrill)
        {
        ...
        if (pitchTypeDrillResultHandler != null)
        {
            pitchTypeDrillResultHandler.currentExpectedAnsString = pitchType + " " + BallType;
            pitchTypeDrillResultHandler.ExpectedResult.Add(pitchType + " " + BallType);
            baseballTransform.gameObject.SetActive(false);
        }

        if (!isVirtualOrPitchtypeDrill)
        {
            if (postupDrillResultHandler != null)
            {
                string ballTypeNew = (BallType == "Strike" ? "Up" : "Down");

                postupDrillResultHandler.BallTypeString.Add(pitchType + " " + ballTypeNew);
                postupDrillResultHandler.isSwingString.Add(isSwing.ToString());

                postupDrillResultHandler.currentAnsString = ballTypeNew;
                baseballTransform.gameObject.SetActive(false);
            }
            if (isSwing == true)
            {
                if (BallType == "Strike")
                {
                    ...
                    if (postupDrillResultHandler != null)
                    {
                        postupDrillResultHandler.ballResult.Add("Correct");
                        postupDrillResultHandler.DecisionText.text = "Correct";
                    }
                }
                else
                {
                    ...
                    if (postupDrillResultHandler != null)
                    {
                        postupDrillResultHandler.ballResult.Add("Incorrect");
                        postupDrillResultHandler.DecisionText.text = "Incorrect";
                    }
                }
    ...
    void UpdateTrajectoryPath()
    {
        ...
        //positions[0] += new Vector3(0, 0, -1); // Shift first point back in Z
        //positions[positions.Length - 1] += new Vector3(0, 0, 1); // Shift last point forward in Z
        ...
        if (positions.Length > 1)
        {
            ...
            for (int i = 0; i < pointCount; i++)
            {
                ...
                {
                    Debug.Log("--> Found point with Z = 2: " + point);
